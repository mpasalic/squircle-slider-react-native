...
class SquirclePathProperties extends SvgPathProperties {
  ...
  getSquirclePointAtPercent(percent: number): {x: number; y: number} {
    ...
    const {x, y} = this.getPointAtLength(
      this.getTotalLength() * adjustedPercent,
    );

    return {
      x: x * ((this.radius * 2) / squircleSVGPathSize.width),
      y: y * ((this.radius * 2) / squircleSVGPathSize.height),
    };
  }
